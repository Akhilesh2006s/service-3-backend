...
const testMongoDBToken = async () => {
  ...
  try {
    ...
    const submissionsResponse = await fetch('https://backend-production-e051.up.railway.app/api/submissions/student', {
      method: 'GET',
      headers: {
    ...
    const examsResponse = await fetch('https://backend-production-e051.up.railway.app/api/exams/student', {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
