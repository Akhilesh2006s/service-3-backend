...
const testSubmissionEndpoint = async () => {
  try {
    ...
    const response = await fetch('https://backend-production-e051.up.railway.app/api/submissions', {
      method: 'POST',
      headers: {
...
const testAuth = async () => {
  try {
    ...
    const response = await fetch('https://backend-production-e051.up.railway.app/api/auth/me', {
      headers: {
        'Authorization': validToken
      }
    });
